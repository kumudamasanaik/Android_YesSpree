...
import com.yesspree.app.customviews.ProductFilterableItems;
...
public class ProductListAdapter extends RecyclerView.Adapter<ProductListAdapter.ViewHolder> implements Filterable {
    ...
    private ProductFilterableItems productFilterableItems;
    ArrayList<ProductData> productFilterdArrayList;

    public ProductListAdapter(Context context, IProductListClickListener iProductListClickListener, RecyclerView recyclerView, String adapterType) {
        this.context = context;
        this.iProductListClickListener = iProductListClickListener;
        this.recyclerView = recyclerView;
        this.adapterType = adapterType;
        this.productDataArrayList = new ArrayList<>();
        this.productFilterdArrayList = productDataArrayList;
    ...
    public void appendList(ArrayList<ProductData> productDataArrayList) {
        this.productDataArrayList.addAll(productDataArrayList);
        notifyItemRangeInserted(this.productDataArrayList.size() - productDataArrayList.size(), productDataArrayList.size());
    }
    ...
    public Filter getFilter() {
        if (productFilterableItems == null) {
            productFilterableItems = new ProductFilterableItems(this, productFilterdArrayList);
        }
        return productFilterableItems;
    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            if (Validation.isValidString(adapterType) && adapterType.equals(Constants.WISHLIST))
                imageWishlist.setVisibility(View.INVISIBLE);
        }
        ...
        private void bindData(int position) {
            ...
            if (adapterType.equalsIgnoreCase(Constants.WISH_LIST_FRAGMENT)) {
                imageWishlist.setVisibility(View.GONE);
            } else {
                setWishlistBtnState(imageWishlist, productData.getWishlist(), productData.getTempWishlistStatus());
            }

            imageWishlist.setOnClickListener(v -> {


                setupWishListData(productData, imageWishlist);
    ...
    public void setApiRespFromModifyWishList(ModifyWishlistResponse modifyWishlistResponse, ProductData wishlistproductData) {
        ...
        try {
            String productId = wishlistproductData.getProductId();
            ...
            if (Validation.isValidObject(modifyWishlistResponse)) {
                current.setWishlist(current.getTempWishlistStatus());
                current.setTempWishlistStatus(-1);
            } else {
                onModifyWishlistError(current, viewHolder);
    ...
    private void onModifyWishlistError(ProductData current, ViewHolder viewHolder) {
        if (current != null) {
            current.setTempWishlistStatus(-1);
            setWishlistBtnState(viewHolder.imageWishlist, current.getWishlist(), current.getTempWishlistStatus());
        }
    }
